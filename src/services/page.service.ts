...
export const getPageDataByType = async (type: string) => {
  const payload = await createPayload(
    ...
    [{ process: "passForm", params: {} }, pageFlattenPostprocess]
  );
...
export const getQueryData = async (query_id: string, paramsData: any) => {
  let params = paramsData ? paramsData : { carouselid: "home-slider" };
  const payload = await createPayload(params, query_id, {
...
export const runQuery = async (
  ...
  const payload = await createPayload(params, query_id, postprocess);
...
export const getUpcomingEventsData = async (query_id: string) => {
  const payload = await createPayload({ numblogs: "2" }, query_id, {
    process: "postStandard",
    params: {},
  });
...
export const getNewsData = async (query_id: string) => {
  const payload = await createPayload({ numblogs: "100" }, query_id, {
    process: "postStandard",
    params: {},
  });
