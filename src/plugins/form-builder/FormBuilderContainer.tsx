...
const FormBuilderContainer = ({ formId, data }: any) => {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchFormFields(applicationId, dataset_id)])
      .then(([fieldsRes]: any) => {
        setFields(
          Object.values(
  ...
  return (
    <>
      {data.blocks_elementid === "contact-form" ? (
        <>
          <ContactForm formId={formId} fields={fields} />
        </>
      ) : (
        <>
